...
void TIM3_IRQHandler(void)
{
  /* USER CODE BEGIN TIM3_IRQn 0 */
	uint16_t status = LL_TIM_ReadReg(TIM3, SR);
	if(status & 0x2) {
		//Canale 1
		LL_TIM_WriteReg(TIM3, SR, LL_TIM_ReadReg(TIM3, SR) & 0xFFFD);
		LL_TIM_WriteReg(TIM3, CCR1, LL_TIM_ReadReg(TIM3, CCR1) + delay1);
	}
	if(status & 0x4) {
		//Canale 2
		LL_TIM_WriteReg(TIM3, SR, LL_TIM_ReadReg(TIM3, SR) & 0xFFFB);
		LL_TIM_WriteReg(TIM3, CCR2, LL_TIM_ReadReg(TIM3, CCR2) + delay2);
	}
	if(status & 0x8) {
		//Canale 3
		LL_TIM_WriteReg(TIM3, SR, LL_TIM_ReadReg(TIM3, SR) & 0xFFF7);
		LL_TIM_WriteReg(TIM3, CCR3, LL_TIM_ReadReg(TIM3, CCR3) + delay3);
	}
...
void TIM4_IRQHandler(void)
{
  /* USER CODE BEGIN TIM4_IRQn 0 */
	if(LL_TIM_ReadReg(TIM4, SR) & 0x10) {
		LL_TIM_WriteReg(TIM4, SR, LL_TIM_ReadReg(TIM4, SR) & 0xFFEF);
		LL_TIM_WriteReg(TIM4, CCR4, LL_TIM_ReadReg(TIM4, CCR4) + 50000);
	}
